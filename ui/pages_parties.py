...
def render():
    st.subheader("Clienti/Fornitori: fatture, pagamenti e saldi")
    ensure_schema()
    ...
    st.markdown("### Riepilogo controparti")
    st.dataframe(grp, use_container_width=True)

    st.markdown("### Dettaglio")
    party = st.selectbox("Controparte", sorted(inv["party"].dropna().unique().tolist()))
    direction = st.selectbox("Tipo", ["emessa","ricevuta"])
    det = inv[(inv.party==party) & (inv.direction==direction)].copy().sort_values("invoice_date", ascending=False)
