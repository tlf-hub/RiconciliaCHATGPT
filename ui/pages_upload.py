...
def _parse_one_xml(blob: bytes, file_id: int, own_vat: str):
    ...
    if kind == "cbi_sdd":
        parse_cbi_sdd(root, file_id); return "SDD_CBI"
    if kind == "pain008":
        parse_pain008(root, file_id); return "SDD_pain008"
    if kind in ("camt053","camt054"):
        parse_camt(root, file_id); return kind
    if kind == "fatturapa":
...
def render(state):
    ...
    st.download_button("⬇️ Template CSV estratto conto", data=_template_csv_bytes(),
                       file_name="template_estratto_conto.csv", mime="text/csv")

    files = st.file_uploader("Carica file (csv/xml/zip)", type=["csv","xml","zip"], accept_multiple_files=True)
    if not files:
        return
    ...
    for f in files:
        ...
        try:
            ...
            if name.endswith(".csv"):
                parse_bank_csv(raw, file_id); parsed += 1
                notes.append(f"✅ {f.name}: CSV")
            elif name.endswith(".xml"):
                kind = _parse_one_xml(raw, file_id, own_vat)
                notes.append(f"{'✅' if kind!='UNKNOWN' else '⚠️'} {f.name}: {kind}")
                parsed += 1 if kind != "UNKNOWN" else 0
                ...
                for n in z.namelist():
                    ...
                    if n.lower().endswith(".csv"):
                        ...
                        notes.append(f"✅ {n}: CSV (zip)")
                    elif n.lower().endswith(".xml"):
                        kind = _parse_one_xml(bb, file_id, own_vat)
                        notes.append(f"{'✅' if kind!='UNKNOWN' else '⚠️'} {n}: {kind} (zip)")
                        parsed += 1 if kind != "UNKNOWN" else 0
